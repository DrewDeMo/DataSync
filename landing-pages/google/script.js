// Google Campaign Landing Page - Dynamic Content Loader

document.addEventListener('DOMContentLoaded', async () => {
    try {
        // Get synced data from localStorage
        const storageKey = 'landing_page_data_google';
        ...
        if (data.synced_at) {
            ...
            document.getElementById('sync-status').textContent =
                `Last synced ${timeAgo} from Google Ads Campaign`;
        } else {
            document.getElementById('sync-status').textContent =
                'Content loaded from Google Ads Campaign';
        }
